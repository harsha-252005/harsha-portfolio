...
export default function Navbar() {
  ...
  return <header className="fixed top-0 z-50 w-full border-b border-[#233554] bg-[#0a192f]/95 backdrop-blur"><nav className="relative mx-auto flex h-14 max-w-[1680px] items-center gap-4 px-5 md:h-[76px] md:gap-8 md:px-12">
    ...
    <div className="hidden items-center gap-6 lg:flex">{links.map(link => <a key={link.label} href={link.href} target={link.external ? '_blank' : undefined} rel={link.external ? 'noreferrer' : undefined} className="text-[16px] font-bold text-[#ccd6f6] no-underline transition hover:text-[#64ffda]">{link.label}</a>)}</div>
