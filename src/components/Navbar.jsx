...
export default function Navbar() {
  ...
  return <header className="fixed top-0 z-50 w-full border-b border-[#233554] bg-[#0a192f]/95 backdrop-blur"><nav className="relative mx-auto flex h-14 max-w-[1680px] items-center gap-4 px-5 md:h-[76px] md:gap-8 md:px-12">
    ...
    <button className="mr-auto ml-0 grid h-10 w-10 place-items-center border-0 bg-transparent text-xl text-[#64ffda] md:ml-auto md:mr-0 lg:hidden" type="button" onClick={() => setOpen(!open)} aria-label="Toggle navigation" aria-expanded={open}>{open ? <FaTimes /> : <FaBars />}</button>
